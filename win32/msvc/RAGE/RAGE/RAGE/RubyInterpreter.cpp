	...
	namespace Interpreter
	{
		...
		Ruby::Ruby(int argc, char** argv)
		{
			...
			ruby_sysinit(&argc, &argv);
			{
				...
				std::string str(al_path_cstr(p, ALLEGRO_NATIVE_PATH_SEP));

				rb_ary_push(rb_gv_get("$:"), rb_str_new_cstr(str.substr(0, str.find_last_of(DS) + 1).c_str()));

				#if RAGE_COMPILE_FS
				if (!PHYSFS_mount(al_path_cstr(p, ALLEGRO_NATIVE_PATH_SEP), "/", 0))
				{
				...
				rb_gv_set(RAGE_EXE_NAME_VAR, rb_str_new2(str.c_str()));
